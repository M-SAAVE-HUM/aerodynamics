...
section_y = np.linspace(0, 1.4, N) # y locations, half span
chords = opti.variable(init_guess=np.ones(N)*0.4) # All chords initially guessed as 0.4
...
opti.subject_to([  # add constraints
    chords > 0.2,  # Chords should stay positive
    wing_linTaper.area() == 0.98,  # fixed area
...
# compute wing area
wing_area = 2*1.5*(root_chord + tip_chord)/2
# Print values
CL = sol(aero["CL"])
CDi = sol(aero["CD"])
print('-'*60)
...
########## DOUBLE TAPER SHAPE OPTIMIZATION ##########

opti = asb.Opti()

N = 3  # double taper (3 chord control points)
section_y_2 = np.linspace(0, 1.4, N)
chords_2 = opti.variable(init_guess=np.ones(N) * 0.35)
