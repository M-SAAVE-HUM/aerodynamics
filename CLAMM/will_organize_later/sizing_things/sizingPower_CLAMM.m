...
etap_climb = 0.70;  % prop efficiency in climb (random guess) [-]


% Estimated design point #1 (based off Kabir's Python Aero Toolbox MDO results)
est1.P0 = 30;        % thrust (random guess) [W]
...
est1.PW0 = est1.P0/est1.W0;    % thrust-to-weight ratio [N/N]
est1.color = 'r';   % dot color

% Estimated design point #2 (most W/S)
est2.P0 = 40;        % thrust (random guess) [W]
est2.m0 = 10.7;      % mass [kg]
est2.W0 = est2.m0*g; % weight [N]
est2.Sref0 = 0.5998; % wing area [m^2]
est2.WS0 = est2.W0/est2.Sref0; % wing loading [N/m^2]
est2.TW0 = est2.P0/est2.W0;    % thrust-to-weight ratio [N/N]
est2.color = 'g';   % dot color

% Estimated design point #2 (most W/S for least T/W)
est3.P0 = 25;        % power (random guess) [W]
...
est3.TW0 = est3.P0/est3.W0;    % thrust-to-weight ratio [N/N]
...
const.PWtakeoff = takeoff(g, rho, dTakeoff, CLmax, const.WSvals);
const.PWclimb = climb(CLmax, CD0, AR, eClimb, G, etap_climb, const.WSvals);
const.PWcruise = cruise(Vcruise, CD0, AR, eCruise, rho, const.WSvals);
const.PWceiling = ceiling(CD0, AR, eCruise);
const.PWmaneuver = maneuver(rho, theta, Vturn, AR, eCruise, CD0, const.WSvals);
...
fig1 = figure(1);
hold on
plot(const.WSvals, const.PWtakeoff,  'LineWidth',2, 'DisplayName','Takeoff')
plot(const.WSvals, const.PWmaneuver, 'LineWidth',2, 'DisplayName','Maneuver')
plot(const.WSvals, const.PWcruise,   'LineWidth',2, 'DisplayName','Cruise')
plot(const.WSvals, const.PWclimb,    'LineWidth',2,'Color','#5c9c0e', 'DisplayName','Climb')
yline(const.PWceiling, 'LineWidth',2,'Color','#0e7d9c', 'DisplayName','Ceiling')
...
scatter(est1.WS0, est1.PW0, 80, est1.color, 'filled', 'DisplayName','Estimated Design Point #1')
...
ylabel('P/W [$W/N$]', 'Interpreter', 'latex', 'FontSize', 16)
...
% [TSfig1] = PS_plot(est1,const);
% [TSfig2] = PS_plot(est2,const);
% [TSfig3] = PS_plot(est3,const);
...
function [fig] = PS_plot(est,const)
    ...
    Ttakeoff = const.PWtakeoff.*West;
    Tclimb = const.PWclimb.*West;
    Tcruise = const.PWcruise.*West;
    Tceiling = const.PWceiling.*West;
    Tmaneuver = const.PWmaneuver.*West;
    ...
    % Estimated design point
    scatter(est.Sref0, est.P0, 80, 'r', 'filled', 'DisplayName','Estimated Design Point')
    
    
    xlabel('$S_{ref}$ [$m^2$]','Interpreter','Latex','FontSize', 16)
    ylabel('$P$ [$W$]','Interpreter','Latex','FontSize', 16)
    legend('Location','northeast');
    xlim([0 6])
    ylim([0 400])
...
% OUTPUTS: PWtakeoff - min. power to weight ratio required for takeoff [-]

function [PWtakeoff] = takeoff(g, rho, dTakeoff, CLmax, WS)
    k = 1.1; % takeoff speed to stall speed ratio
    SF = 1.5; % safety factor to consider drag/other deficiencies
    Vto = k*sqrt(2*WS./(rho*CLmax)); % Takeoff speed
    etap = prop_eff(Vto); % Prop efficiency
    PWtakeoff = (Vto./etap).*(SF.*WS.*k^2) ./ (g*rho*dTakeoff*CLmax);
end
...
%          etap - prop efficiency at climb [-]
% OUTPUTS: PWclimb - min. power to weight ratio needed for climb [-]

function [PWclimb] = climb(CLmax, CD0, AR, e, G, etap, WS)
    ks = 1.2; % ratio of climb speed to stall speed
    CLclimb = CLmax/(ks^2); % CL at climb
    G = tand(G); % climb gradient percent

    PWclimb = (8.84.*(WS.^(1/2))./(etap)).*(G+((CD0 + CLclimb^2/(pi*AR*e)) / CLclimb))/(CLmax^(1/2));
...
%          rho - air density [kg/m^3]
%          WS - vector of wing loading values [N/m^2]
% OUTPUTS: PWcruise - power to weight ratio needed for cruise [-]

function [PWcruise] = cruise(Vcruise, CD0, AR, e, rho, WS)
    q = 0.5*rho*(Vcruise^2); % dynamic pressure [Pa]
    etap = prop_eff(Vcruise); % Prop efficiency

    PWcruise = q.*Vcruise.*(CD0 + (WS.^2)/(q^2*pi*AR*e))./(etap.*WS);
end
...
%          CD0 - parasitic drag coefficient [-]
%          WS - vector of wing loading values [N/m^2]
% OUTPUTS: PWmaneuver - power to weight needed for banked turn [-]

function [TWmaneuver] = maneuver(rho, theta, Vturn, AR, e, CD0, WS)
    n = 1/cosd(theta); % load factor
    q = 0.5*rho*Vturn^2; % dynamic pressure
    etap = prop_eff(Vturn);
    TWmaneuver = ((q*CD0)./WS + WS.*(n^2 / (q*pi*AR*e)))*Vturn/etap;
end


%% Propeller efficiency curve

function [etap] = prop_eff(V)
    % INPUT:   V - freestream velocity [m/s]
    % OUTPUT:  etap - propeller efficiency
    ...
    etap = 0.75; % Need surrogate/parametric equation for eta_p in terms of J
end